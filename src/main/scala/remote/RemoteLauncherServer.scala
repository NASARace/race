...
/**
  ...
  */
class ServerLauncherOpts (var poolSize: Int = 5) extends SSLLauncherOpts {
  identity = Some(new File(identityDir,"id_rsa"))
  ...
  override def check: Boolean = {
    // we run non-interactive, so we need both
    checkExistingFileOption(identity, "identity file") &&
    checkExistingFileOption(keyStore, "keystore file")
  }

  override def parser = new ServerLauncherOptsParser("launcherserver")

  class ServerLauncherOptsParser (name:String) extends SSLLauncherOptsParser(name) {
    head("launchserver", "1.0")
    opt[Int]("poolsize").text(s"thread pool size for concurrent requests (default=$poolSize)").optional()
      .action{(v,opts) => poolSize = v; opts}
  }
}
