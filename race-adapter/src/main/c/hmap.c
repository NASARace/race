 ...
 * 
 * The implementation is a vairation of https://github.com/anholt/hash_table.git
...
static inline bool is_available_entry(hmap_entry_t* e) {
  return e->key == NULL;
}
...
static bool rehash (hmap_t* map) {
  int const_idx = map->const_idx+1;
  if (const_idx < ARRAY_SIZE(map_consts)) {
    ...
    for (hmap_entry_t* e = hmap_next_entry(map,NULL); e; e = hmap_next_entry(map,e)) {
      ...
      while (!is_available_entry(new_e)){
...
static inline bool check_rehash (hmap_t* map) {
  if (map->n_entries >= map->consts.max_entries) {
    return rehash(map);
  } else {
    return true;
  }
...
static inline void reloc_entry (hmap_t* map, int idx, uint32_t h, hmap_entry_t* e_from, hmap_entry_t* e_to ) {
  ...
  e_from->key = NULL;
  e_from->hash = is_next_entry_free(map,idx,h) ? 0 : DELETED_HASH;
  e_from->data = NULL;
}
...
bool hmap_add_entry (hmap_t* map, const char* key, void* data) {
  ...
  for (int idx = h % map->consts.size; true; idx = next_index(map,idx,h)) {
    ...
    if (is_available_entry(e)) {  // new entry
      e->key = key;
      e->hash = h;
      e->data = data;
      map->n_entries++;
...
hmap_entry_t* hmap_get_entry (hmap_t* map, const char* key) {
  ...
  for (int idx = h % map->consts.size; true; idx = next_index(map,idx,h)) {
    ...
    } else if (e_reloc == NULL && is_deleted_entry(e)) { // move it up if we still find it
      e_reloc = e;
    }
  }
}
...
bool hmap_remove_entry (hmap_t* map, const char* key) {
  ...
  for (int idx = h % map->consts.size; true; idx = next_index(map,idx,h)) {
    ...
    if (is_free_entry(e)) {
      ...
      e->key = NULL;
      e->hash = is_next_entry_free(map,idx,h) ? 0 : DELETED_HASH;
      e->data = NULL;
...
hmap_entry_t* hmap_next_entry (hmap_t* map, hmap_entry_t* prev_entry) {
  ...
  for (hmap_entry_t* e = e_first; e != e_last; e++) {
    if (!is_available_entry(e)) { // skip over free and deleted entries
      return e;
    }
  }
  return NULL;
