...
import gov.nasa.race.swing.DigitalClock
...
class BasicClockPanel(raceView: RaceView, config: Option[Config]=None) extends GBPanel {
  ...
  layout(new Label("simulation time:").styled('labelFor))  = c(0,0).weightx(0.5)
  layout(simClockPanel)                                    = c(1,0).weightx(0)
  layout(new Label("elapsed time:").styled('labelFor))     = c(0,1).weightx(0.5)
  layout(simStopWatchPanel)                                = c(1,1).weightx(0)
