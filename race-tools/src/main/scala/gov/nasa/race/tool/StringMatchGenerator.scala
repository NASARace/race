...
class PrefixTree {
  ...
  class Node(var value: String, // this does not include the prefix (combined parent values)
    ...
    @tailrec protected final def computePrefixLength (l: Int): Int = {
      if (parent.isDefined) {
        val n = parent.get
        n.computePrefixLength(n.value.length + l)
      } else {
        value.length + l
      }
    }

    def combinedPrefixLength: Int = if (parent.isDefined) parent.get.computePrefixLength(0) else 0
...
object StringMatchGenerator {
  ...
  def main (args: Array[String]): Unit = {
    if (args.length < 2) {
      ...
      println("  gen    generate function implementing recognizer")
    }
    ...
    cmd match {
      ...
      case "gen" => generateRecognizer(strings)
  ...
  def generateRecognizer(strings: Array[String]): Unit = {
    val tree = new PrefixTree
    strings.foreach(tree += _)
