...
object TextToTaggedArchiveConverter {
  ...
  class Opts extends CliArgs(s"convert text archive to tagged archive format") {
  ...
  def main (args: Array[String]): Unit = {
    if (opts.parse(args)) {
      for (inFile <- opts.inFile;
        ...
        processArchiveEntries(new TextArchiveReader(is, inFile.getPath), new TaggedStringArchiveWriter(os,outFile.getPath,32768))
        is.close
      }
    }
  }
