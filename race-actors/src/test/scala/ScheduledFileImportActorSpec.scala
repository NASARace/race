...
import gov.nasa.race.core.RaceActorSpec.Continue
  ...
  "a ScheduledFileImportActor" must {
    "publish all scheduled messages" in {
      runRaceActorSystem(Logging.WarningLevel) {
        val sched = addTestActor(classOf[ScheduledFileImportActor], "scheduler", createConfig(
          """ name = "fileImporter"
              schedule = "race-actors/src/test/scala/schedule.xml"
