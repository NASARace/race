...
/**
 * JMS (SWIM) node simulator that uses plain JMS/ActiveMQ
 */

package gov.nasa.race.swimserver

import com.typesafe.config.{ Config, ConfigFactory }

import javax.jms.{ Connection, TextMessage, Session, Topic, MessageProducer }
...
object MainSimple {
  ...
  def main (args: Array[String]) = {
    System.setProperty("logback.configurationFile", "logback-swimserver.xml") // need our own to not collide with race
    ...
    try {
      ...
      try {
        ...
        case ex: Throwable => println("MainSimple failed to shut down")
      }
    }
  }
}
