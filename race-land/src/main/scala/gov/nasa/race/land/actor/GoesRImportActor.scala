...
import gov.nasa.race.land.{GoesRData, GoesRDataReader, GoesRProduct}
    ...
    products.foreach { product =>
      ifSome(getLatestObject( now,product)) { obj=>
        ifSome(downloadObject( obj, product)){ file=>
          ...
          sendToClient( GoesRData( satellite, file, product, DateTime.ofInstant(obj.lastModified())))
        }
      }
    }
  }
...
class GoesRImportActor (val config: Config) extends PublishingRaceActor {
  ...
  val satellite = config.getString("satellite")
  ...
  override def onInitializeRaceActor(rc: RaceContext, actorConf: Config): Boolean = {
    if (s3Client.isDefined && products.nonEmpty) {
      val thread = new GoesRDataAcquisitionThread( self, satellite, interval, s3Client.get, products, dataDir)
