...
#[derive(Clone)]
struct ChildState<A: MsgReceiver<ChildResponse>> { parent: A }

impl_actor! { match msg: ChildMsg for ChildState<A> where A: MsgReceiver<ChildResponse> as 
    ChildRequest => cont! {
        sleep(secs(1)).await;
...
impl_actor! { match msg: ParentMsg for ParentState as 
    _Start_ => cont! { 
...
async fn main() ->Result<()> {
    ...
    let actor_handle = spawn_actor!( actor_system, "parent", ParentState::new())?;
