...
import gov.nasa.race.common.{ASCII8Internalizer, ASCIIBuffer, ClearableElementsHolder, Internalizer, UTF8XmlPullParser2}
...
class AsdexMsgParser(val config: Config=NoConfig) extends UTF8XmlPullParser2
            with ConfigurableTranslator with ClearableElementsHolder[ArrayBuffer[AsdexTrack]] {
  ...
  override def createElements = new ArrayBuffer[AsdexTrack](150)
  ...
  def parse (bs: Array[Byte], off: Int, limit: Int): Option[Any] = {
    ...
    if (airportId != null && elements.nonEmpty) Some(AsdexTracks(airportId,elements)) else None
  }
  ...
  def parseTracks (bs: Array[Byte], off: Int, limit: Int): AsdexTracks = {
    if (initialize(bs,off,limit)) {
      ...
      AsdexTracks(airportId,elements)
    } else AsdexTracks("",Seq.empty[AsdexTrack]) // FIXME
  }
