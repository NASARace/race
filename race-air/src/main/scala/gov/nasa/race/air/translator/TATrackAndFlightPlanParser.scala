...
import gov.nasa.race.common.{ASCII8Internalizer, ASCIIBuffer, ClearableElementsHolder, UTF8XmlPullParser2}
...
class TATrackAndFlightPlanParser(val config: Config=NoConfig)  extends UTF8XmlPullParser2
           with ConfigurableTranslator with ClearableElementsHolder[ArrayBuffer[TATrack]] {

  val allowIncompleteTrack: Boolean = if (config != null) config.getBooleanOrElse("allow-incomplete", false) else false

  override def createElements = new ArrayBuffer[TATrack](150)
  ...
  def parseTracks(bs: Array[Byte], off: Int, limit: Int): Seq[TATrack] = {
    if (initialize(bs,off,limit)) {
      ...
      if (elements.nonEmpty) elements else Seq.empty[TATrack]
  ...
  def parseTracks(s: Slice): Seq[TATrack] = parseTracks(s.data,s.offset,s.limit)
