...
/**
  ...
  */
class TATrackAndFlightPlanParser(val config: Config=NoConfig)  extends UTF8XmlPullParser2 with ConfigurableTranslator {

  val allowIncompleteTrack: Boolean = if (config != null) config.getBooleanOrElse("allow-incomplete", false) else false

  var tracks: ArrayBuffer[TATrack] = null // set during initialization
  ...
  def parseTracks(bs: Array[Byte], off: Int, limit: Int): Seq[TATrack] = {
    if (initialize(bs,off,limit)) {
      while (parseNextTag) {
        if (isStartTag) {
          if (tag == ns2TATrackAndFlightPlan || tag == taTrackAndFlightPlan) {
            parseTATrackAndFlightPlan
          } else return Seq.empty[TATrack]
        }
      }

      if (tracks.nonEmpty) tracks else Seq.empty[TATrack]
  ...
  protected def clearTracks: Unit = {
    if (tracks != null && reUseTracks) {
      ...
      tracks = new ArrayBuffer[TATrack](150)
    }
  }
