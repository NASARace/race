...
class ItwsMsgParser (val config: Config=NoConfig)
  ...
  protected def parseItwsMsg: Option[Any] = {
    ...
    while (parseNextTag) {
      if (isStartTag) {
        ...
        @inline def process_product_header = parseProductHeader
        @inline def process_precip = parsePrecip
  ...
  def readImage (scanLine: Array[Byte], img: BufferedImage): Unit = {
    if (parseSingleContentString){
      ...
      splitter.setSource(contentString)
