...
object ConsoleIO {
  ...
  private val menuStack: Stack[MenuCall] = new Stack()
  private val globalLock = new ReentrantLock

  private def loopReader(currentThread: Thread): Boolean = {
    while (currentThread != menuStack.top.thread) { // stack cannot shrink below currentThread 
      if (menuStack.top.thread.getState() == Thread.State.WAITING){
        // nobody else notifies but the reader, so no further sync required
        menuStack.top.synchronized {
          menuStack.top.notify  // wake up follower, keep reading from currentThread
          return true
        }
      }
      Thread.`yield`
  ...
  def menu (prompt: String, action: =>Any = {})(cmds: MenuFunc): Unit = {
    ...
    globalLock.lock                           // (12)
    menuStack.pop                             // (13)
    if (!menuStack.isEmpty && (t ne menuStack.top.thread)){
      print(s"\n$menuColor${menuStack.top.prompt}$resetColor")
      scala.Console.flush
    }
