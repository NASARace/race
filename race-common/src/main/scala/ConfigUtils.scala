...
object ConfigUtils {
  ...
  implicit class ConfigWrapper(val conf: Config) {
    ...
    def getConfigArray (key: String): Array[Config] = {
      try {
        Array(conf.getConfigList(key):_*)
      } catch {
    ...
    def getMapOrElse(key: String, fallBack: Map[String,String]): Map[String,String] = {
      try {
        conf.getConfig(key).entrySet().foldLeft(Map.empty[String,String]) { (acc,e) =>
  ...
  implicit class ConfigListWrapper(val configList: java.util.List[Config]) {
    def asList: List[Config] = {
      var list: List[Config] = Nil
      for (c <- configList) list = c :: list
      list
    }
  }
}
